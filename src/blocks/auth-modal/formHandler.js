...
export function submitSigninForm(data) {
    return fetch('/wp-json/edb/v1/signin', { // Replace with the actual endpoint path if needed
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        ...
        .then(data => {
            if (data.status === 1) {
                return { success: true, message: 'Thank you for signing in!' };
            } else {
                return { success: false, message: data.message || 'An error occurred during signup.' };
            }
        })
